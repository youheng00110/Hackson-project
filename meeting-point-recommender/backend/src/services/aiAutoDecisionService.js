    ...
    async decide({ city, districtId, participants, candidates }) {
        ...
        const formatCandidate = (item) => {
            ...
            return [
                ...
                `• ${item.trafficReason}`,
                `• ${item.weatherReason}`,
                `• ${item.reputationReason}`
            ].join('\n');
        };

        const rankingLines = top3.map((item, index) => `• ${index + 1} ${item.name}`);
        const reasonBlocks = top3.map(item => formatCandidate(item));

        const summary = best
            ? `综合路况与天气，更推荐 ${best.name}。通行时间更短且周边拥堵更低，天气不佳时也更友好。`
            : '当前暂无可用推荐。';
